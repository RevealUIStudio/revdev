...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    untracked: { label: '?', cls: 'bg-surface-1/20 text-fg-muted' },
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {dir && <span className="block truncate text-[10px] text-fg-subtle">{dir}</span>}
      </div>
  ...
  return (
    <button
      ...
      className={`flex size-5 items-center justify-center rounded transition-colors ${
        danger
          ? 'text-fg-subtle hover:bg-error/40 hover:text-error'
...
function CommitLog({ entries, loading }: { entries: GitCommitInfo[]; loading: boolean }) {
  ...
  return (
    <div className="h-full overflow-y-auto">
      {entries.map((entry) => (
        <div key={entry.sha} className="border-b border-edge/60 px-3 py-2 hover:bg-surface-1/40">
          <div className="flex items-start gap-2">
            <span className="shrink-0 rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[10px] text-fg-muted">
              {entry.short_sha}
            </span>
            <span className="min-w-0 flex-1 text-xs leading-snug text-fg-muted">
              {entry.message}
            </span>
          </div>
          <div className="mt-0.5 flex items-center gap-2 pl-8 text-[10px] text-fg-subtle">
...
export default function GitPanel({ onOpenEditor }: GitPanelProps) {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* ── Left pane: file list ── */}
      <div className="flex w-72 shrink-0 flex-col border-r border-edge bg-surface-0">
        {/* Header */}
          ...
          {editingPath ? (
            ...
            <>
                ...
                <div ref={branchDropdownRef} className="relative min-w-0 flex-1">
                  ...
                  {showBranchDropdown && (
                    <div className="absolute left-0 top-full z-50 mt-1 w-56 rounded-lg border border-edge bg-surface-2 shadow-xl">
                      ...
                      {branchError && (
                        <p className="mx-2 mb-1 truncate rounded bg-error-subtle px-2 py-1 text-[10px] text-error">
                          {branchError}
                        </p>
                      )}
              ...
              <button
                ...
                className="mt-0.5 max-w-full truncate text-left text-[10px] text-fg-subtle hover:text-fg-muted"
              ...
              {remoteError && (
                <div className="mt-0.5 flex items-start gap-1 text-[10px] text-error">
                  ...
                  <button
                    ...
                    className="shrink-0 rounded px-1 leading-none text-error hover:bg-error-subtle hover:text-error"
        ...
        {/* Status error */}
        {statusError && (
          <div className="mx-3 mt-2 rounded border border-error/50 bg-error-subtle px-2.5 py-2 text-[10px] text-error">
            {statusError}
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {!(status || loadingStatus || statusError) && (
            <p className="px-2 py-4 text-center text-xs text-fg-subtle">No repository loaded</p>
          )}
          {loadingStatus && !status && (
            <p className="px-2 py-4 text-center text-xs text-fg-subtle">Loading…</p>
          )}
          {status && totalFiles === 0 && (
            <p className="px-2 py-4 text-center text-xs text-fg-subtle">Working tree clean</p>
          )}
          {status && (
            <>
              <Section title="Staged" count={status.staged.length} accent="text-success">
              ...
              <Section title="Changes" count={status.unstaged.length} accent="text-warning">
              ...
              <Section title="Untracked" count={status.untracked.length} accent="text-fg-subtle">
