/**
 * Studio-local principal for bridge kg_* tools.
 *
 * Same hook-identity files SessionStart writes
 * (`REVDEV_HOOK_IDENTITY_DIR` / `~/.local/share/revealui/hook-identities`).
 * Does not import `@revealui/mcp` or `@revealui/harnesses`.
 */
...
function missingPrincipalKey(): string {
  return process.env.REVDEV_AGENT_ID?.trim() || `ppid:${process.ppid}`;
}

export function warnMissingPrincipal(): void {
