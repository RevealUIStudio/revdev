...
|---|---|---|
| Studio | Buildable; OS-unsigned | Local: `pnpm --filter studio tauri build`. `studio-release.yml` is cutting public tags — latest `studio-v0.2.12` (2026-08-30). Updater pubkey + signing secrets configured 2026-06-11 (H1). The release workflow reads Apple/Microsoft OS code-signing secret names (H5) and stays unsigned while they are empty. Remaining: H4 custom-domain mirror (`releases.revealui.com` still 404s) and the owner-set certificates (H7/H8). |
...
| H5 | CI secrets for signed builds | **PARTIAL — workflow reads secret names (GAP-273)** | Updater `TAURI_SIGNING_PRIVATE_KEY{,_PASSWORD}` set (H1). `studio-release.yml` reads `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_PASSWORD`, `APPLE_TEAM_ID`, `WINDOWS_CERTIFICATE`, and `WINDOWS_CERTIFICATE_PASSWORD`. Those OS cert secrets are still owner-set; an empty value keeps that platform unsigned. No certificate material is in the repo. |
